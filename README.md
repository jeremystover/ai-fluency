...
- **The podcast** (`/module/1/podcast`) is one made-for-you episode per module, not a studio the learner has to operate. The scriptwriter gets the module's content blocks (exercises excluded — you can't read JSON aloud) plus everything known about the listener — name, role, selected goals, investment depth, diagnostic direction — and is told to make the episode unmistakably theirs: greeted by name, examples fit to their role, goals addressed mid-episode, length set by their depth (quick/standard/deep). Maya asks, Leo explains (`podcast-v4` prompt, `PODCAST_MODEL` + prompt version persisted on `fd_podcast`). The script also carries a 3–6 beat **outline** (`outline_json`), rendered as a follow-along rail beside a compact transcript — beats light up with playback and click-to-seek, both estimated by character proportion. **Cost logic:** anyone who picked podcast as a learning style gets their episode **pregenerated in the background — script and voices** — at intake, and again on each `module_completed` so the next module's episode is play-on-arrival ready; nothing is generated further ahead than that, since most learners won't finish everything. For everyone else, arriving at the podcast page is the signal: generation starts unprompted (opening the studio *is* picking the modality), and there are no buttons between them and playback. After listening (gated server-side on `podcast_played`), the learner can **ask the hosts questions**: a `kind: 'qa'` follow-up segment (quick length, mailbag format, grounded in the module, questions inside `<listener_questions>` tags as steering data, not instructions). The Q&A hosts also remember what this listener actually heard: the module episode's script and the last three follow-ups ride along in `<heard_episode>` blocks, so answers can point back to the exact episode ("like we said about…") and build on earlier answers instead of repeating them. One default episode per module; free-form regeneration is gone. **Audio streams in chunks:** the script is cut at speaker-turn boundaries into ~5 parts (the first ~10% of the speech, so playback starts in seconds — `shared/audioChunks.ts`, one deterministic plan shared by worker and player). The background render (`waitUntil` after every script, manual or pregenerated) fills the R2 cache chunk by chunk in playback order on Workers AI `@cf/deepgram/aura-2-en` (thalia/orpheus — $0.03/1k chars, ~15–35¢ per episode); the player fetches chunk 0 immediately — served from cache or voiced live in ~10–20s, whichever wins — then chains parts gaplessly at turn boundaries, always fetching one ahead. There is no polling and no whole-episode wait anywhere. Custom controls: play/pause, char-proportion progress bar with click-to-seek, elapsed/total (estimated until all chunks load), and a playback-speed cycle (1×/1.25×/1.5×/2× — persisted per browser). Episodes voiced before chunking keep their single cached MP3 (`audioMode: 'single'`). No key → honest offline message; no AI binding → transcript-only. 4 scripts/hour/session covers manual, Q&A, and pregeneration alike. Funnel: `podcast_requested` (with `kind` + `trigger: manual|pregen`) `→ podcast_script_ready → podcast_audio_rendered → podcast_played`.
