...
import VisualCard from './EpisodeVisual';
import { api, ApiError, track } from '../api';
import { useApp } from '../brand';
import { chunkPlan, type AudioChunk } from '../../shared/audioChunks';

// The full Fluent episode experience, embedded in the module page: one-flow
// generation, instant assembled playback, follow-along outline, takeaways,
// the concept sketch, and listener Q&A. Mount it only when the learner asks
// to listen (or listening is their style) — mounting IS asking for the
// episode, so a hidden mount would generate episodes nobody requested.
...
// The listener's map: the episode's main beats, lit up as playback passes
// them. Clicking a beat jumps the audio there (by the same character-
// proportion estimate the transcript highlight uses). Rendered inline above
// the transcript — the module page's single column has no room for a rail.
function FollowAlong({
  ...
  return (
    <div className="border border-line rounded-brand bg-surface p-5 mt-5" aria-label="Episode outline">
      ...
      <ol className="flex flex-col">
        {outline.map((pt, i) => {
          ...
          return (
            <li
              key={i}
              className={`relative border-l-2 pl-4 pb-3 last:pb-0 ${
  ...
  return (
    <div className="mt-5">
      <div className="border border-ink-strong rounded-brand bg-surface p-5">
        ...
        <h3 className="font-display font-bold text-ink-strong text-2xl mt-2">{episode.title}</h3>
...
export default function PodcastPanel({ moduleId }: { moduleId: string }) {
  const { me } = useApp();
  ...
  useEffect(() => {
    api
      ...
      .then(setList)
      .catch((e) => setError(e instanceof ApiError ? e.message : 'The podcast did not load. Reload to try again.'));
  }, []);
  ...
  // One flow: opening this panel IS asking for the episode. If none exists
  ...
  if (!list && !error) {
    return <p className="label-utility mt-4">Opening your episode…</p>;
  }
  ...
  return (
    <div>
      <p className="text-ink text-sm mt-2">
        ...
        talk through this module for one listener: you. Your name, your role, your goals shape the episode.
      </p>

      {list && !list.scriptEnabled && (
        <div className="mt-5"><ErrorNote message="The scriptwriter is not configured in this deployment, so episodes cannot be generated yet." /></div>
      )}
      ...
      {defaultEp && list?.scriptEnabled && (
        <div className="border border-line rounded-brand bg-surface p-5 mt-6">
