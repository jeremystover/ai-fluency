...
You will go and build that list in the activity. It is the artifact this module exists to produce.
...
does not, and the disclosure problem that is yours alone.
...
- B. That the obligations are legal but the evidence that discharges them is statistical — Legal can state the standard and cannot tell you whether you met it ✓
...
- B. That it generalises — any geographic, educational or behavioural feature can function as a proxy, and strict liability means intent is not a defence, so feature-by-feature proxy review is a modelling task ✓
...
- D. Because consent for survey participation cannot cover model training
...
> **B.** Two failure modes: *someone reached a document they shouldn't have* versus **nobody's
...
- D. Because open-text responses cannot be suppressed by cell size
