...
**True of us, or not true of us?** If you think it isn't true, name where the condition is written
down and who would find it.

You'll check during the activity. **If you cannot find it in writing, "we all know it" is the
...
Here's the thesis, and it is the whole course in one line:

> **The expert move is stopping the model, not accelerating it.**

Everything else here's machinery for making that affordable. Because "be more rigorous" is advice
...
There is a second reason this module matters more than it looks. **You're about to be the bottleneck
for other people's analysis, not just your own.** When an HRBP produces a regression, you're the one
who has to say whether it holds. Doing that case by case doesn't scale, and doing it by reputation
...
You already have one of these and it isn't this one.

The comp track's craft module gives the construction rule: **never present a number you could not
...
necessary here and it is **assumed, not repeated.** If you haven't got that discipline, this module
is premature.

This module's rule is its sibling and governs the layer above:

> **Never present a finding whose alternative explanations you have not stated.**
...
The pattern this module teaches isn't aspirational. It is published, deployed, and open-sourced, and
...
**That division is the module's thesis, built by people shipping it at scale.** They didn't automate
...
**"Credibility level" is the output shape to copy.** Not *here's your effect estimate* but *here's
...
threshold, the analysis stops and reports that it stopped — it doesn't report a number with a wider
...
the normal case in HR and which breaks naive difference-in-differences in ways that weren't widely
understood until recently. *Halt:* if rollout timing varies and the design doesn't account for it,
stop.

**Contested estimands.** The question doesn't have one answer because it doesn't have one meaning.
*What is the effect of the manager programme*: on whom, compared to what, over what horizon, for
those who took it or those offered it? *Halt:* if the estimand isn't written down and agreed before
the analysis runs, the analysis doesn't run. **This is the most common failure on the list and the
least technical.**
...
**Performance data is a rating produced by the system you're evaluating.** Use it as the outcome in
a study of that system and you have built the circularity the comp track's exclusion zone warns
about, one function over. If ratings carry the disparity you're looking for, controlling for them
removes the finding.
...
the halt conditions earn their keep, because the failure isn't statistical. It lands on a named
person.
...
predicts "nobody leaves" scores 88%. So 78% accuracy isn't evidence of anything, and the questions
...
*And what would falsify it?* Here's the honest answer, and the module states it rather than papering
...
on the prediction and cleanly evaluate it**, and every honest design here's a compromise: a holdout,
...
This module has spent five lessons on stopping. Here's the affirmative half, and it is the
...
account.** It won't volunteer the confound, not because it is hiding one, but because nothing in
...
- **Two rules, two layers.** Comp M3 governs construction — *never present a number you couldn't
  ...
  you have not stated.** A construction failure gets caught eventually. **An inference failure has no
...
the only available proxy for something real; the estimand that can't be agreed in advance because the
stakeholder doesn't yet know what they are asking. **A file of thresholds gives a junior analyst
...
that it is there next time, under deadline, when you aren't the one running it.

**Step 1 — The estimand (5 min).** Write the question the analysis answers, precisely enough that two
people couldn't disagree about what would count as an answer. On whom, compared to what, over what
...
- C. Because cautions can't be encoded in code
...
- B. Very little — predicting "nobody leaves" would score 88%, so accuracy against a low base rate isn't evidence, and the questions that matter are the flag rates and what a manager does after three false positives ✓
