...
> Almost nothing here's volatile. These failure modes are properties of how the models work, not
...
*against its source.* Someone reading the draft carefully, and liking it, isn't what this claims.
...
A model hasn't read the plan documents. It has read a great deal *like* the plan documents, which
...
The flattening isn't a quality problem: the prose is competent. It is an **identity** problem.
...
employment, where carryover isn't available" is the actual policy. The summary isn't false about
...
**Your ear is the wrong instrument, and it is wrong in proportion to how good you're at this job.**
...
finding: a draft with no source is a draft nobody can verify, and it shouldn't be reviewed. It
should be sent back.
...
> ### Try this — 3 minutes
> Take any AI-assisted piece you published in the last month and open its source next to it. Do not
...
The only way to catch it is to have the source open and be reading for absence — which isn't what
reviewing feels like and isn't what anybody does under time pressure.
...
real person, traceable to your channel, and they won't learn it was your omission. They will
learn that what you publish can't be relied on.
...
Three properties make a stop-rule hold, and they are the same three that made 101 M7's lines hold:

**Named, not roled.** "Comms reviews it" isn't a rule. A person's name is a rule.
...
- **Two failure modes 101's taxonomy does not have**, because neither is an error: **voice
...
function that reviews everything carefully and arrives late hasn't managed risk — it has moved it
...
know you should stop but will not, and why.** Naming it honestly is the strongest form of this
...
- C. It isn't an error at all — the prose is accurate and competent. It is an identity problem: the channel stops sounding like your organization, gradually enough that no single message is worth objecting to ✓
...
> **B.** "Employees may carry over up to five days" survives; "except in the first year of employment" does not. The summary is not false about what it says. It's false about what it omits, and it harms the new joiners least equipped to know.
...
- D. Because it can't detect factual errors
...
- C. Because sources contain information the draft can't include
...
- D. Omissions can't be corrected after publication

> **B.** The consequences differ in the same direction as the detectability. A fabricated number embarrasses the function and gets corrected. A missed enrollment window costs an employee money, and they never learn it was your omission, only that your channel cannot be relied on.
...
- B. Because instance rules can't answer a case they didn't anticipate, and next month always brings one ✓
- C. Because named instances create single points of failure
- D. Because categories are easier to audit for compliance

> **B.** "The benefits email gets checked" has nothing to say about the message that is not on the list. Categories answer new cases, which is the entire reason to write them that way — alongside being named rather than roled, attached to a source, and carrying a stated fallback.

## Sources and attribution

- No external statistic carries this module, and none should. The failure modes
