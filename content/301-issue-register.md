...
| **A-2** | **Organisational Development / Org Design — explored; provisional yes, gated on verification.** See `content/course-301-org-development-exploration.md`. Opposite result to A-1, for specific reasons: the subtraction leaves **four** substantial uncovered areas (operating model design, the re-org as a governed event, structure options appraisal, and role/work decoupling) rather than three overlapping ones. **The decisive line is against 401, not the 301 siblings** — 401 teaches you to keep a structure standing (its capstone is literally *What We're Keeping Standing*); it never designs a target state. That is the same conversation-versus-architecture distinction that made Comp a track. **Both proposed merges assessed and declined:** TD & OD gives a 6-module track a second unrelated spine and two audiences; HRBP is the reactive-versus-architectural error in reverse, and HRBP is already the longest track and the `other` fallback. **Three reservations:** the audience threshold (~5,000 people) is the highest of any track and is a market question for the course owner; 401 already serves much of this learner, so the boundary needs a stated tripwire; and **verification has not been run** in a field whose literature is the most consultancy-dominated in HR. **Sequence: verify → outline → build, not straight to outline.** Three placements owed regardless, filed as CP-9, CP-10, CP-11. | **explored — decision owed on the market question** |
