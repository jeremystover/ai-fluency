...
| **S-6** | **Conflicting AI-policy statistics across tracks.** Comp M4: *68% of employers have a formal AI policy* (Littler, May 2026). People Ops M3: *51% have no formal AI use policy* (SHRM). Two instruments, arguably compatible — but a learner cannot see both and a reviewer can. Reconcile once, centrally, via the evidence library. | `ai301-comp-m4`, `ai301-peopleops-m3` | open |
| **S-7** | **`ai201-m8` has no interactive exercise.** The only module in the curriculum with neither `sorting.json` nor `choice.json`. Every other module across 13 courses has one. | `content/modules/ai201-m8/` | open |
| **S-8** | **Point-of-claim sourcing is thinner than the curriculum's own standard in places.** Comp M2 declares *"the numbers, with their samples attached — because a module about reading evidence has to model it"*, then states Gartner's 88% bare in the body (its ~114-leader sample appears 60 lines later as a teaching example); the 68% Littler figure carries no sample or date where it is claimed; *"about a quarter"* and *"44% of US workers"* have no attribution anywhere. See C-06. | `ai301-comp-m2`, `ai301-comp-m4`; audit others | open |
...
| **R-5** | **Shared evidence library.** Stage 1 (extraction, clustering, divergence report) — see `content/evidence/`. Stage 2: extend `scripts/maintenance-agent.mjs` to check the library entry against the web once, then each citing block against the library entry, so divergence is flagged without a search. | in progress |
