...
| **S-8** | **Point-of-claim sourcing is thinner than the curriculum's own standard — and it is curriculum-wide, not a comp-track problem.** The library pass measured it: **92 of 189 claims (~49%)** in the 101/201/HRBP/recruiter/comp group carry no named source at the point of claim, including whole clusters of hard percentages in `ai301-hrbp-m1` and `ai301-recruiter-r1`, and most of the legal blocks in the other groups. The authoring rule is now in brief §4 for new content; this is the shipped backlog. | curriculum-wide; `npm run sourcing:check` | **in progress — measured, tooled, worst offenders fixed.** `scripts/sourcing-check.mjs` finds figures in volatile blocks with no attribution in the window a reader actually has — back to the nearest sub-heading and forward one paragraph, using each module's **own** sources block as the source list, so nothing needs maintaining as the curriculum grows. **67 → 23 flagged paragraphs.** Fixed: `ai301-hrbp-m1` L2 (the highest-traffic lesson in the curriculum — vague quantifiers replaced with 31% / 54% / 87% / 92% carrying SHRM's 1,722 sample and December 2025 field dates, plus a caution that *expecting more adoption is not expecting a lot of it*) and `ai301-recruiter-r1` (Greenhouse and the ~19,368-interview analysis named at the point of claim). **The residual ~23 includes an irreducible class the tool cannot judge** — figures used illustratively rather than evidentially (`analytics-m2`'s hypothetical 78% attrition score, `cpo-m6`'s quoted decision rule). Treat the output as a review list, not a defect list. Remaining real ones: `talent-dev-m6`, `ai401-m8`, `comp-m4`, `ler-l1`, `ler-l8`. |
...
| ID | Item | Status |
|---|---|---|
| **X-1** | **Access control vs. disclosure control** — the split is agreed from both sides but written in only one. People Ops M6 owns *who can retrieve what* (failure: someone reached a document they shouldn't have; fix: permissions, indexing scope, least-privileged testing). Analytics M5 L5 owns *what a legitimately-permissioned aggregate reveals* (failure: nobody's permissions were violated and the individual was identified anyway; fix: thresholds and suppression). **Both tracks should carry the sentence.** | **closed — already satisfied in both directions; this entry was stale.** Verified by reading, not grepping: `ai301-peopleops-m6` states the boundary in Lesson 3 *and* records it as a horizontal note in its sources; `ai301-analytics-m5` carries the reciprocal and names the split as a cross-track convention agreed with People Ops. Nothing to write. |
| **X-2** | **Comparator vs. control group.** LER draws it — one legal comparator can be dispositive where one data point is nothing. Analytics does not yet carry the reciprocal note, and a learner taking both would import statistical intuitions into a legal analysis. Cited in brief §3a as *the* example worth copying. | **closed** — `ai301-analytics-m2` Lesson 1 gains *"And one place where a different rule governs entirely"*: a single similarly-situated comparator can be dispositive, so the statistical instinct that n=1 is nothing is correct statistically and wrong legally. On a comparator search the analyst's job is **retrieval and completeness, not inference**, and volunteering *"the sample is too small"* answers a question nobody asked. **Note the entry's own framing was wrong** — it said *a learner taking both tracks*, but §3a means no learner sees two. The real exposure is an analyst asked to support an ER matter, which is how it is now written. |
| **X-3** | **Analytics co-determination delta.** BetrVG §87(1) no. 6 turns on whether a system is *objectively suitable* for monitoring — **intent is irrelevant** — so "we're only analysing data we already have" is not a defence. People Ops M7 covers co-determination for *deployment*; it does not carry the analytics version where the analysis output is itself the monitoring device. | **closed — already satisfied; this entry was stale.** `ai301-analytics-m5` Lesson 6 opens on §87(1) no. 6 BetrVG with **objectively suitable** and *the employer's intent is irrelevant*, names manager dashboards and workforce analytics as examples, and kills *"we're only analysing data we already have"* explicitly. Its sources block records the split with People Ops M7. |
| **X-4** | **Case-history index.** LER wants the investigation corpus searchable for comparator analysis; People Ops M6 names exactly that corpus as one a general assistant must never index. Both are right. The reconciliation — scoped to ER team access, never joined to the enterprise assistant, existence itself a discovery consideration — belongs in the content of both. | **closed** — only the People Ops side was owed. `ai301-ler-l5` Lesson 5 already carried the full reconciliation (ER-team scope, never joined to the enterprise assistant, structured extracts over narrative reports, and raise it with counsel **before** building). `ai301-peopleops-m6` Lesson 4 now carries the other half: the ER corpus exclusion is right and is **not** the same as refusing the capability, so arrive at the challenge with the scoped alternative already drawn — the challenge comes from the function with the strongest case for an exception. |
...
| **X-6** | **Works-council consultation.** People Ops M7 owns it as a deployment dependency, which is right for a systems owner. In Europe it is core labor-relations work. Under §3a the answer is probably **both, specialised differently** — deployment consultation there, bargaining obligation in LER. | **closed — already satisfied, and specialised exactly as predicted.** `ai301-ler-l7` Lesson 3 covers the US duty to bargain (mandatory subjects, *Endurance* on management-rights waiver, and the honest point that **no Board decision has yet applied it to an AI deployment**); `ai301-peopleops-m7` covers EU works-council consultation under the BetrVG. LER's sources block records the division in both directions. |
