...
Two things to commit, and the second one is a number you'll check against reality inside this
module.
...
the mistake isn't about AI at all. It is about what you inventory.

The instinct is to audit **tasks**: list what your team does, find the repetitive parts, and point
AI at those. That instinct is correct and 201 M1 taught you how to act on it, because when you're
...
pick, which exception path exists and which doesn't. Each one was decided once, encoded, and then
ran for years without anyone revisiting it. A task audit doesn't see them, because from the
...
decision-rights map with a signature line. All of it fails at the first step if you can't say who
decides.
...
Here's the shape of the thing you're looking for. Somebody decided, at some point, that leave
...
defend this if it were challenged?** Not the team. The person. If you can't, it is a deferred
...
decision doesn't create the accountability gap. **It inherits one that was already there and makes
it move faster.**
...
Put those together and the picture is specific. **You're the front line of a function that mostly
has not started.** More than half of organizations have deployed nothing; where anything has been
deployed, it is disproportionately in your territory; and the executives above you're near
...
currently account for it.** Nobody sells a product for that, which is why you're reading a course
about it instead.
...
drafting assistant in the letter templates, an agent that answers policy questions. You didn't
...
Read that against Lesson 2 and notice they don't contradict each other. SHRM asked where AI is
being used across HR practice areas and found your function second. Sapient asked organizations
about their own systems estate and found a third can't say what is in it. Different questions,
...
You aren't evaluating at this stage. You're producing a list of things that need evaluating,
...
can't reconstruct the rationale is a different risk from one with no owner, and it is treated
differently later.
...
That last quadrant is where the damage lives. And here's the part worth writing on a wall:
...
provider isn't an undo. **It is a correction project with a blast radius of its own**, and the
...
produce evidence. So if you do automate one, the audit question arrives immediately and it isn't
about accuracy: **where is the record that it ran, and who looked at it?** A reminder nobody
acknowledged isn't a control. It is a log entry that will be read out loud to you later.
...
- **The diagnostic question is "can you name the person who would defend this?"** Not the team. If
  you can't, it is a deferred decision — and AI enabled on top of one inherits an accountability
...
- **Three doors, one gate.** A feature in a system you already own (biggest, no gate), a new product
  ...
  on the middle one. Nearly a third of organizations can't say whether their current HR systems
...
The strongest counter-argument isn't that the claim is false. It is that **it is true of all
enterprise technology and has never been a problem before.** You didn't evaluate your payroll
...
free. On that view, demanding an evaluation for every AI item in a release note isn't governance,
...
shape a decision about a person need a gate**, and most release-note items don't. A better
...
answer is "it depends, case by case, and I'll know it when I see it," you haven't built a gate.
You have described yourself as one, and you don't scale.
...
reversibility, accountability **by name**, and defensibility. **Write "do not know" and "nobody"
where they are true** — those entries are the most valuable output of this exercise and they are
what Module 7 will need. Don't resolve a blank by naming a team.
...
- B. Because the things carrying risk in a systems role are decisions that were made once, encoded, and abandoned while still executing, and a task audit can't see them, since an encoded decision looks like a process step ✓
...
> the point: you're the front line of a function that mostly has not started, which makes
> "we have not adopted AI yet" usually false in your stack specifically.
...
- B. Because a team can't sign anything, and the track's final artifact is a decision-rights map with a signature line ✓
...
**Q8.** What does "AI can prepare and remind, but it can't be the control" require in practice?
...
- **SHRM, *The State of AI in HR 2026***, the practice-area adoption split (recruiting 27%, HR
  ...
  aligned with `ai301-hrbp-m2`, which uses the same source, so the two don't drift. **[V]**
- **Sapient Insights Group, *28th Annual HR Systems Survey Report* (2025–2026)** — 31% of
  ...
  applications with embedded AI 9%), and the finding that nearly one-third of organizations don't
...
- **A note on what is not here.** We looked for a credible measurement of HR data quality to use in
  this track and couldn't find one with a disclosed sample. Module 5 says so out loud, because the
