...
AI-assisted content already live in your catalog? Give a number. You'll produce the real one in
the activity, and the gap is the finding.

If nothing in your catalog was AI-assisted, or you can't tell which items were, **write that
down.** "We can't identify which of our live content was AI-assisted" is a finding of the first
...
terms, and analogies to legal citation research. What you don't find is a rate. There is no
...
something about itself. It also means the honest thing this module can offer you isn't a
...
**Here's the mechanism, stated up front.** 101 M6 gave you four ways AI gets things wrong:
...
true. The error isn't in any sentence. It is in the premise the sentences were built on, and you
supplied that.
...
4. Run a backward read on live content, starting from the instruction rather than the first
   sentence.
5. Write a premise-challenge instruction, and say why it isn't a complete defence.
...
**The model is not ignorant. It is deferential.** Which means the fix isn't a better model, and
waiting for one isn't a strategy. It also means the capability you need is already available to
...
one of them accepted. The better you're at your job, the more freight your brief carries, and the
...
**It passes. Correctly.** The reviewer did their job. The job doesn't include the premise, because
the premise came from you, and reviewers don't audit the brief. They audit the output against the
brief.

This is why the failure is *silent* rather than merely undetected. Your process doesn't miss it by
accident or through carelessness. **Your process is not looking at the layer where the error lives**,
and adding more of the same review won't help. Three careful accuracy passes catch three times as
...
doesn't merely escape their check. **It confirms their expectations**, which is the strongest
...
There is no published taxonomy for this, so here's ours. Five patterns, defined by the property
...
**2 · Laundered authority.** A real source, correctly cited, for a claim it doesn't make. The
...
traced to a study. The citation is real. The attribution is real. The claim isn't.
...
These aren't obscure errors. **They are what a well-read professional in this field believes**,
...
excellent and omits a content element your jurisdiction mandates isn't inaccurate. It is
...
> about your learners — what they know, why they are not doing something, how they prefer to learn.
...
1. **Name the instruction.** What does this content want someone to do differently? State it in one
   sentence. If you can't, that is the finding: a module with no locatable instruction isn't
...
published study has measured it. You're about to generate your own data point.
...
substantially reduces the deferential behaviour. You're converting a request back into a question,
...
can't be checked by a model with no access to your organization. It will say so if asked, which is
why the second clause is in the instruction, but saying so isn't detecting.

It is defeatable by your own framing. Assert something strongly enough and agreeableness reasserts
itself. The instruction reduces the effect; it doesn't eliminate it.
...
- **Backward read for what's live**, start at the instruction and work back to what supports it.
  **Premise challenge for what is not built** — and it has three real limits, including that it can't
...
a prior isn't a citation. **That may be exactly the reasoning a competent reviewer would use to
...
regardless, and note in the write-up that you couldn't determine provenance. **That is a finding
and it is graded as one** — a function that can't tell which of its live content was
...
do differently? If you can't locate one, stop and write that down. You have found something larger
...
- B. A fabricated citation to a study that doesn't exist
- C. A real source, correctly cited, used to support a claim it doesn't actually make ✓
...
- B. It requires a specialized system prompt most teams can't configure
- C. It catches what the model knows is contested, can't check claims about your own organization, and can be overridden by a strongly asserted framing ✓
...
- **Dale's Cone of Experience** — published 1946 with no retention percentages and an explicit
  ...
  attribution to the National Training Laboratories hasn't been traced to a source study. **[V]**
