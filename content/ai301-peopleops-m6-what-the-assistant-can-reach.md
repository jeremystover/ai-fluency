...
**And the number you'll check:** the activity asks you to run **ten questions from a
...
that user is permitted to see.** If a user can't open a file, the assistant can't open it on their
behalf. The index doesn't create new access.
...
And the mistakes aren't exotic. They are the ordinary residue of a decade of normal work:
...
doing all the decisive work. Access controls were never your only protection. They were the
...
isn't survivable at the new one. **"AI did not cause this" is true and is not a defence**. It removed
...
reaching people who weren't in them: come largely from **security vendors who sell remediation**,
and their counts aren't independently verified. Reason from the mechanism. Don't build your
...
candid commentary. Restructure planning documents naming people who haven't been told. Legal-hold
...
ratio. The controls aren't wrong; they are the right size for the wrong distribution, and nothing in
...
Now the genuinely new failure mode, which isn't about permissions at all.
...
Here's the shape of it. A user asks a general question — *what's the thinking on the Commercial
reorg?* The assistant answers well, drawing on several sources. The answer contains the substance of
a planning document that user would never have located and wouldn't have opened. And:

- **No filename.** They can't cite what they learned, and neither can you.
...
- **No entry in any audit log saying they read it.** They didn't read it. They read an answer.
...
access. There is none, for anybody. The trail you have spent a career learning to follow doesn't
...
Nobody over-shared *that*. The assistant assembled it, and the assembly isn't in anyone's index of
sensitive documents because it didn't exist until it was asked for.
...
people. You're the function that runs reports for everybody, and now the reporting interface accepts
natural language from people who don't know what a suppression threshold is.
...
right**, and yours isn't yet. The principle is one the Comp & Benefits track states in a different
...
available to you and the discipline's most common error, for a reason that is almost funny: **you're
an administrator. You can see everything. You're therefore the worst possible tester**, and every
...
- **Every over-share you already had is now one prompt away.** The exposure existed; the effort that
  hid it didn't survive. So **"AI did not cause this" is true and is not a defence**, the permissions
  problem was survivable at the old discovery cost and isn't at the new one.
- **Reason from the mechanism, not from security-vendor incident counts.** The mechanism is
  documented; the dramatic numbers come from people selling remediation and aren't independently
  verified.
...
- **Five moves:** inventory what is indexed separately from what the HCM permits; **exclusion before
  labelling**, because the most reliable protection for a document isn't being in the index; **test as
  a least-privileged persona, never as yourself**; enforce a minimum group size on the reporting
  surface and verify it refuses; and where you don't own the tenant, **bring the exclusion list, not
  the concern.**
...
The strongest counter-argument isn't that the claim is wrong. It is that **the problem is real and it
...
HRBP track's legal module faces (*this isn't your job, you're absorbing work belonging to functions
...
designated test account.** Don't probe production systems with a colleague's credentials, don't use
...
mid-level individual contributor with no HR role. If you can't obtain one, say so, name who could
provide it and by when you'll have asked. That blocker is itself a finding worth reporting.
...
**Step 4. The exclusion list (5 min).** Name the sites or repositories that shouldn't be indexed —
...
something that account shouldn't have reached.
...
opening claim turned out to be true of your organization; and the concrete next step — **who you're
taking the exclusion list to, and what you're asking them to do with it.**
...
- B. Because it is true and settles nothing, the permissions problem was survivable at the old discovery cost and isn't survivable at the new one ✓
- C. Because permissions and AI risk can't be separated technically
- D. Because it shifts responsibility to IT, who will refuse it

> **B.** "AI did not cause this" is true and is not a defence. Removing the friction that made a latent
...
- D. That the assistant can't cite its sources reliably
...
- B. That the user isn't culpable, so there is no deterrent
...
- D. That audit logs can't be reconciled
...
- A. Because labelling requires licences that exclusion doesn't
- B. Because labelling only protects you to the extent it is correct, and yours isn't yet, while the most reliable protection for a document isn't being in the index ✓
...
- B. Because you're an administrator who can see everything, so every result you get is reassuring and means nothing — the vendor's own guidance is to validate exposure per persona ✓
...
**Q8.** For a practitioner who doesn't administer the assistant tenant, what does the module recommend?
...
- **Reported HR-specific incidents** — compensation figures returned to benign questions, AI summaries
  ...
  the module reasons from the mechanism instead, and doesn't use the incident counts. **[V]**
