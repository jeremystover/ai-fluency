...
Procurement is where the terms are set, where you have leverage you'll never have again, and where
a question costs nothing. After signature you're asking a vendor to do you a favour. Before it, you
are a buyer.
...
because the single most common failure isn't skipping the diligence. It is **accepting a document
...
Which doesn't make the numbers false. It makes them a claim with an interest attached, and the
...
benchmark, because you can't show the sample, the population or the question wording.
...
not proof of a bad product; it is proof you can't rely on the claim, which is what you needed to
know.
...
sources beyond what they submitted, retains it, or uses it across employers, you're in territory
that may implicate consumer-report obligations regardless of whether the model is fair. **Ask it as
a data question, not a fairness question**, because the fairness answer won't surface it.
...
management practices of a defined shape. It is procedural, organizational, and it doesn't have an
outcome to come back bad.

**ISO/IEC 42001** is the standard you'll be shown, and it is worth understanding precisely because
...
and governance; it doesn't certify the fairness or performance characteristics of any particular
...
practical consequence: the employer doesn't get to say "that was the vendor's system," and the
vendor doesn't get to say "we only make the software." Responsibility for the outcome sits with the
...
- **Silence is an answer.** A question that produces no document has told you that you can't rely
...
- **An audit measures a system's outcomes; a certification attests that an organization runs a
  ...
  review) not the fairness of any model**, and isn't equivalent to EU AI Act compliance. A vendor
...
fairness audit on your population requires your data, which you can't give a vendor before you buy;
audits on the vendor's population are of limited relevance to you; and small vendors (often the
ones building the most careful products) can't fund per-customer validation. On that reading a
management-system certification isn't a substitution but a **reasonable proxy for the thing you
...
four questions, phrased so a non-answer is recognizable. Plus what you'll accept as an answer to
...
- C. They shouldn't be used at all, since vendor research is unreliable
...
- B. Because the fairness answer won't surface it — and if the system assembles information from beyond what the candidate submitted, you may be in consumer-report territory regardless of whether the model is fair ✓
...
> **B.** This is the *Kistler* question. That case does not allege the algorithm was biased. It alleges an FCRA disclosure failure, which is a liability route that runs entirely around fairness.

**Q4.** What makes "what would we need from you if we were sued?" worth asking before signature?

- A. It signals to the vendor that you're a sophisticated buyer
...
> **B.** Can they produce the model version in effect on a given date, the configuration you ran, and the contemporaneous audit? That is either written into the contract or it is not.
...
> **C.** It shows the vendor's response-to-results mechanism works, which is the criterion Connecticut weighs and the thing Module 3's protocol is built around. A clean audit demonstrates nothing about what happens when it is not clean.
...
- D. That contract terms can't bind model behaviour
...
- **ISO/IEC 42001:2023** — an AI management system standard specifying requirements for
  ...
  organizational processes and governance, not the fairness or performance of a model, and isn't
