...
**The claim:** *"Your AI has already told an employee something that wasn't true, and you can't say
which employee."*

**Is that true of your organization?** *True of us* or *not true of us*, and the one sentence you
would defend it with. If you have no employee-facing AI at all, answer for the one you'll have.
This module is mostly about what to do before you switch it on.

**And the number you'll check:** the activity asks you to put **ten real questions** through your
...
health, in the company's voice. An employee who reads an answer from your service desk doesn't
experience it as a language model's output. They experience it as **what the company told them**, and
they act on it.

And there is a second asymmetry that nothing else in this curriculum has. Your capability doesn't
...
and one honest metric. The bad news is that the parts that aren't settled aren't settled by
...
Start with the number your vendor will put on the slide, and why you shouldn't accept it.

**Deflection** counts contacts that didn't reach a human. It is the standard measure for
...
All three look identical from the outside. The person didn't come back to a human, so all three
...
employees can't switch providers, and they have to come back. And the 14% is the figure traceable
to Gartner's own release, while the 45%-deflected pairing reaches us second-hand. **What transfers is
the mechanism, not the arithmetic**, don't put 31 points on a slide as though it were your number.
...
figure and it hasn't been independently audited, the same standard this curriculum applies to
IBM's widely-repeated attrition-savings claim, which it treats as unverifiable rather than false.

But notice which number is actually useful to you. It isn't the 94%. **It is the 6%.** That residual
is where every hard case lives, and it is the entire legal surface of this module. A design that
handles 94% beautifully and routes the remaining 6% badly hasn't solved the problem; it has
concentrated it.
...
They didn't come back. They didn't complain. They are a satisfied number in your deflection rate
...
isn't a candidate who never knew. It is an employee who now believes something false about their own
...
Here's the failure inventory, and every item is boring:
...
The model didn't get it wrong. Your corpus did, and the model repeated it faithfully. That
...
remove them. Retrieval doesn't respect your intentions about which copy is canonical; it respects
...
document set isn't ready for a topic, **narrow the capability's scope to the topics that are** —
Module 2's rule, arriving with teeth. Launching broadly across an ungoverned corpus doesn't produce
...
Your capability receives input from employees. Some of that input isn't a question. It is, in legal
terms, **notice.**
...
interaction**, and where the chatbot doesn't respond appropriately, the employer **may** face a
...
employment counsel, not holdings, and we didn't find a controlling decision on chatbot-as-notice.
That isn't a reason to discount them — it is the reason the gate at the top of this module exists.
The exposure is identified by the people who defend these claims for a living, and the boundaries
are unsettled.

The operational consequence doesn't depend on how the law resolves:

> **Notice to the bot is notice to you.**

Which converts escalation routing from a design preference into a control. It isn't about employee
...
And then the hard part, which is why most implementations of this list don't work:
...
reading something they didn't need to. Under-routing costs you a statutory obligation nobody
noticed. Those aren't symmetrical and the design shouldn't treat them as though they were.
...
notes. Now it is **logged, at scale, with timestamps.** That is genuinely the best evidence you'll
ever have that your function treats people the same way, and it is the best evidence anyone else
will ever have that it doesn't. The log isn't neutral; it is a record that will be read by whoever
...
all is caught by this simply by operating a chatbot. If your service desk assistant doesn't announce
itself, that is a same-quarter fix and not a roadmap item.

**Access.** An AI-first support channel that a screen-reader user can't complete is an access
...
- **Deflection counts an abandoned conversation and a confidently wrong answer identically to a
  ...
  45%-deflected pairing reaches us second-hand. The mechanism transfers; the arithmetic shouldn't go
  on your slide.
...
- **Three corpus controls:** one source per topic with the others *removed from the index*, an
  ...
  per topic. If the corpus isn't ready, **narrow the scope** rather than launching broadly.
...
**The claim:** *"Deflection measures that they stopped asking you. It doesn't measure that they got
...
handbook can't escalate an accommodation request, has never once noticed that somebody mentioned a
medical condition, and nobody has ever measured whether a reader acted on it correctly. HR has
deflected for thirty years. On this view the assistant isn't a new legal category at all — **it is a
...
not read, quote, or paste any real employee's conversation, and don't submit anything
...
from a stale document is this module's central failure and it won't look like a failure.
...
specific commitment — **which document you're removing from the index this month, and who has to
...
- D. It can't be benchmarked against other organizations
...
**Q3.** Why does the module say the model isn't the variable for this use case?
...
- C. Because model quality can't be evaluated without benchmarks
- D. Because vendors don't disclose which model they use
...
> **B.** Because it converts an invisible failure into a refusal, a capability that says "I cannot
...
- B. Because people don't use the words — "I'm struggling to get in for 9am since my treatment started" is an accommodation request containing neither the word accommodation nor disability ✓
- C. Because keyword lists can't be maintained across languages
- D. Because models can't reliably detect keywords in long messages
...
- **This module is not legal advice.** It is a guide to which questions to take to your own counsel,
  and Lesson 3 shouldn't be acted on without them.
